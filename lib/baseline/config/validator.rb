...
module Baseline
  class Config
    ...
    # warmup counts). Raises Baseline::ConfigurationError on any problem,
    ...
    module Validator
      ...
      def check_unknown_keys(hash, schema, path)
        hash.each_key do |key|
          ...
          unless value.is_a?(Hash)
            raise Baseline::ConfigurationError, "expected #{(path + [key]).join(".")} to be a mapping"
          end
      ...
      def check_known_key(key, schema, path)
        ...
        raise Baseline::ConfigurationError, "unknown configuration key: #{(path + [key]).join(".")}"
      end

      def check_version(hash)
        return if hash[:version] == 1

        raise Baseline::ConfigurationError, "unsupported configuration version: #{hash[:version].inspect} (expected 1)"
      end
      ...
      def check_execution(hash)
        ...
        if samples && (!samples.is_a?(Integer) || samples < MINIMUM_SAMPLES)
          raise Baseline::ConfigurationError,
        ...
        raise Baseline::ConfigurationError, "execution.warmup must be a non-negative integer, got #{warmup.inspect}"
      end
    end
  end
end
