  ...
  module Storage
    ...
    class Filesystem < Adapter
      ...
      def load_run(reference)
        ...
        raise Baselined::ResultBundleError, "no run result found at #{run_json_path}" unless File.exist?(run_json_path)
      ...
      def export_archive(run_id, into: @root)
        dir = run_directory(run_id)
        raise Baselined::ResultBundleError, "no run bundle found at #{dir}" unless File.directory?(dir)
      ...
      def verify_checksum!(dir, run_json_path)
        ...
        raise Baselined::ResultBundleError, "checksum mismatch for #{run_json_path}"
      end
    end
  end
end
