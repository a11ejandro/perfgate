  ...
  class Config
    class << self
      ...
      def read_yaml(path)
        ...
        raise Baselined::ConfigurationError, "invalid YAML in #{path}: #{e.message}"
      end
    ...
    # with a callable of their own via `Baselined.configure`.
    DEFAULT_DATASET_FINGERPRINT = -> { ENV.fetch("BASELINED_DATASET_VERSION", "unspecified") }
