...
RSpec.describe "Baseline RSpec integration" do
  ...
  before do
    Baselined::Workloads::Registry.reset!
    Baselined.configuration = Baselined::Config.default
  end

  def define_sandboxed_example(metadata = { baselined: true }, &block)
  ...
  it "discovers a :baseline-tagged example as a workload" do
    ...
    Baselined::RSpec::Discovery.call

    expect(Baselined.registry.ids.size).to eq(1)
  end
  ...
  it "does not register examples without :baseline metadata" do
    ...
    expect(Baselined.registry.ids).to be_empty
  end
  ...
  it "runs the discovered workload repeatedly and serializes a completed result" do
    ...
    Baselined::RSpec::Discovery.call
    workload = Baselined.registry.first
    workload_config = Baselined.configuration.execution_defaults

    result = Baselined::Execution::Runner.new(workload).call
    run_result = Baselined::Serialization::RunResult.build([result])
  ...
  it "honors an explicit id override in the :baseline metadata" do
    define_sandboxed_example(baselined: { id: "custom-id" }) { 1 }

    Baselined::RSpec::Discovery.call

    expect(Baselined.registry.ids).to eq(["custom-id"])
  end

  it "raises a configuration error when two examples share an explicit id" do
    define_sandboxed_example(baselined: { id: "same-id" }) { 1 }
    define_sandboxed_example(baselined: { id: "same-id" }) { 2 }

    expect { Baselined::RSpec::Discovery.call }.to raise_error(Baselined::WorkloadError, /duplicate/i)
  end
end
