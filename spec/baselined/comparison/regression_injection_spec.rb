...
require "baselined/comparison/engine"
require "baselined/policy/engine"
require "baselined/config"
...
RSpec.describe "regression injection" do
  let(:config) { Baselined::Config.default }
  ...
  def workload(id:, duration:, sql_count: nil, definition_hash: "sha256:same")
    ...
    summary = { "duration_ns" => Baselined::Statistics::Summary.call(duration) }
    summary["sql_count"] = Baselined::Statistics::Summary.call(sql_count) if sql_count
  ...
  def compare_and_evaluate(baseline_run, candidate_run)
    comparison_result = Baselined::Comparison::Engine.compare(
      baseline_run: baseline_run, candidate_run: candidate_run, config: config
    )
    policy_result = Baselined::Policy::Engine.evaluate(comparison_result: comparison_result, config: config)
