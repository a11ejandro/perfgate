...
RSpec.describe Perfgate::Config do
  describe ".default" do
  ...
  describe ".load" do
    ...
    it "rejects unknown top-level keys" do
      Dir.mktmpdir do |dir|
        ...
        expect { described_class.load(path) }.to raise_error(Perfgate::ConfigurationError, /unknown configuration key/)
      end
    end
    ...
    it "rejects unknown nested keys" do
      Dir.mktmpdir do |dir|
        ...
        expect { described_class.load(path) }.to raise_error(Perfgate::ConfigurationError, /execution\.bogus/)
      end
    end
    ...
    it "rejects a samples count below the spec's minimum of 3" do
      Dir.mktmpdir do |dir|
        ...
        expect { described_class.load(path) }.to raise_error(Perfgate::ConfigurationError, /samples/)
      end
    end
    ...
    it "rejects an unsupported configuration version" do
      Dir.mktmpdir do |dir|
        ...
        expect { described_class.load(path) }.to raise_error(Perfgate::ConfigurationError, /version/)
      end
    end
    ...
    it "applies BASELINE_-prefixed environment variable overrides" do
      Dir.mktmpdir do |dir|
        ...
        with_env("PERFGATE_EXECUTION_SAMPLES" => "15") do
          config = described_class.load(path)
