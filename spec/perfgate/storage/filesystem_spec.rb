...
require "stringio"
require "zlib"
require "perfgate/storage/filesystem"

RSpec.describe Perfgate::Storage::Filesystem do
  let(:run_result) do
    {
      "schema_version" => 1,
  ...
  describe "#load_run" do
    ...
    it "raises ResultBundleError when the run does not exist" do
      expect { storage.load_run("does-not-exist") }.to raise_error(Perfgate::ResultBundleError, /no run result/)
    end
    ...
    it "raises ResultBundleError when the checksum has been tampered with" do
      ...
      expect { storage.load_run(run_result["run_id"]) }.to raise_error(Perfgate::ResultBundleError, /checksum mismatch/)
    end
  end
  ...
  describe "#export_archive / #import_archive" do
    ...
    it "raises ResultBundleError when exporting a run that does not exist" do
      expect { storage.export_archive("does-not-exist") }.to raise_error(Perfgate::ResultBundleError, /no run bundle/)
    end
    ...
    it "rejects an archive entry that attempts path traversal on import" do
      ...
      expect { storage.import_archive(archive_path, into: @root) }
        .to raise_error(Perfgate::ResultBundleError, /escapes the destination directory/)
    end
  end
