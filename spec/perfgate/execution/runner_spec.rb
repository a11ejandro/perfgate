...
RSpec.describe Perfgate::Execution::Runner do
  def workload(samples: 3, warmup: 1, metrics: [:duration], &block)
    Perfgate::Workloads::Workload.new(id: "w", samples: samples, warmup: warmup, metrics: metrics, &block)
  end
  ...
  it "prefers an explicit Perfgate.measure duration over the wall-clock measurement" do
    load_result = described_class.new(
      workload(samples: 1, warmup: 0) { Perfgate.measure { nil } }
    ).call

    expect(load_result["samples"].first["duration_ns"]).to be >= 0
  end

  it "only collects allocation counts for code inside the Perfgate.measure block" do
    result = described_class.new(
      workload(samples: 1, warmup: 0, metrics: %i[duration allocations]) do
        ...
        Perfgate.measure { Array.new(3) { Object.new } }
      end
    ).call
  ...
  it "reports no non-duration metrics when Perfgate.measure is never called" do
    result = described_class.new(
