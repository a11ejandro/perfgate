...
RSpec.describe Perfgate::Workloads::Registry do
  subject(:registry) { described_class.new }

  let(:workload_a) { Perfgate::Workloads::Workload.new(id: "a", samples: 3, warmup: 0, metrics: [:duration]) { 1 } }
  let(:workload_b) { Perfgate::Workloads::Workload.new(id: "b", samples: 3, warmup: 0, metrics: [:duration]) { 2 } }
  ...
  it "rejects a second workload with a duplicate id" do
    registry.register(workload_a)
    duplicate = Perfgate::Workloads::Workload.new(id: "a", samples: 3, warmup: 0, metrics: [:duration]) { 3 }

    expect { registry.register(duplicate) }.to raise_error(Perfgate::WorkloadError, /duplicate/i)
  end
