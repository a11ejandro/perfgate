...
RSpec.describe Baseline::Config do
  describe ".default" do
    it "matches the defaults documented in the spec" do
      ...
      expect(config.storage_path).to eq(".baseline")
    end
  ...
  describe ".load" do
    ...
    it "merges user overrides on top of the defaults" do
      Dir.mktmpdir do |dir|
        path = File.join(dir, "baseline.yml")
    ...
    it "rejects unknown top-level keys" do
      Dir.mktmpdir do |dir|
        ...
        expect { described_class.load(path) }.to raise_error(Baseline::ConfigurationError, /unknown configuration key/)
      end
    end
    ...
    it "rejects unknown nested keys" do
      Dir.mktmpdir do |dir|
        ...
        expect { described_class.load(path) }.to raise_error(Baseline::ConfigurationError, /execution\.bogus/)
      end
    end
    ...
    it "rejects a samples count below the spec's minimum of 3" do
      Dir.mktmpdir do |dir|
        ...
        expect { described_class.load(path) }.to raise_error(Baseline::ConfigurationError, /samples/)
      end
    end
    ...
    it "rejects an unsupported configuration version" do
      Dir.mktmpdir do |dir|
        ...
        expect { described_class.load(path) }.to raise_error(Baseline::ConfigurationError, /version/)
      end
    end
    ...
    it "applies BASELINE_-prefixed environment variable overrides" do
      Dir.mktmpdir do |dir|
        ...
        with_env("BASELINE_EXECUTION_SAMPLES" => "15") do
          config = described_class.load(path)
