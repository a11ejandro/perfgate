...
RSpec.describe Baseline::Workloads::Registry do
  subject(:registry) { described_class.new }

  let(:workload_a) { Baseline::Workloads::Workload.new(id: "a", samples: 3, warmup: 0, metrics: [:duration]) { 1 } }
  let(:workload_b) { Baseline::Workloads::Workload.new(id: "b", samples: 3, warmup: 0, metrics: [:duration]) { 2 } }
  ...
  it "rejects a second workload with a duplicate id" do
    registry.register(workload_a)
    duplicate = Baseline::Workloads::Workload.new(id: "a", samples: 3, warmup: 0, metrics: [:duration]) { 3 }

    expect { registry.register(duplicate) }.to raise_error(Baseline::WorkloadError, /duplicate/i)
  end
