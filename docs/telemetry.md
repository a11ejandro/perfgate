...
Perfgate collects **no telemetry by default**. This document specifies
...
whether Perfgate is safe to run in a sensitive environment -- can see
...
(see `lib/perfgate/config/defaults.rb`), but no code path currently
...
- Telemetry is off unless a user explicitly sets `telemetry.enabled:
  true` in `perfgate.yml`, or an equivalent explicit environment
  override.
- There is no implicit opt-in through usage, installation, or CI
  execution.
- Perfgate must function identically, with no missing features and no
  degraded behavior, whether telemetry is enabled or not.
- A telemetry send failure (network error, timeout, malformed
  response) must never affect Perfgate's exit code, comparison result,
...
- IP-derived geolocation, beyond whatever is inherent to receiving an
  HTTP request (Perfgate itself never resolves or stores this)

## Why this split

The permitted fields are enough to answer aggregate product questions
("which Ruby/Rails versions are people actually running Perfgate
